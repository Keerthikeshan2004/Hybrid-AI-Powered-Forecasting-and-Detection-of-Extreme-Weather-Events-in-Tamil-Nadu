...
def run_pipeline():
    ...
    # --- PHASE 1: HYBRID MODEL & DATA FUSION ---
    print("-" * 51, "PHASE 1: HYBRID MODEL & DATA FUSION", "-" * 51)
    ...
    if os.path.exists(MODEL_PATH):
        try:
            ...
            print("✅ Hybrid Model Trend Generated.")
        except Exception as e: 
            print(f"⚠️ Hybrid Model Error: {e}")

    # --- PHASE 2: AI AGENT EXECUTION ---
    print("-" * 51, "PHASE 2: AI AGENT EXECUTION", "-" * 51)
    ...
    print("\n⚡ Processing Districts (Fusing Live Data + Model Trend)...")
    ...
    for district, data in sorted_districts:
        ...
        # C. AI AGENT PREDICTION
        ai_msg, ai_rain_val = "UNKNOWN", 0.0
        if risk_agent:
            risk_result = risk_agent.predict_risk(
                lat, lon, 
                external_current_rain=current_rain, 
                hybrid_model_rain=hybrid_trend_val 
            )
        ...
        report_rows.append({
            ...
            "pred_rain": val_api,     # Added Raw API Forecast
            "trend": hybrid_trend_val,
            "ai_rain": ai_rain_val, 
            "risk_msg": ai_msg, 
            "coords": (y_idx, x_idx), "lat": lat, "lon": lon
        })
    ...
    for row in report_rows:
        ...
        if row['ai_rain'] > 60:
            ...
            flood_risk_list.append((row['name'], {"lat": row['lat'], "lon": row['lon'], "rain": row['ai_rain']}))

        # Updated Row Format with PRED column
        print(f"{row['name']:<18} | {row['current']:<8.2f} | {row['pred_rain']:<8.2f} | {row['trend']:<10.2f} | {status:<15} | {row['ai_rain']:<10.3f} | {ai_status:<15} | {risk_msg}")
    
    print("-" * 140)

    print("-" * 53, "PHASE 3: FLOOD DETECTION MODULE", "-" * 54)
    try: Flood_detection.run_forecast()
    except Exception as e: print(f"⚠️ Flood Module Failed: {e}")

    print("-" * 140)
    print("-" * 53, "PHASE 4: CYCLONE TRACKING MODULE", "-" * 53)
    cyclone_tracks = Cyclone_prediction.run_cyclone_system()

    print("-" * 140)
    print("-" * 58, "PHASE 5: GENERATE MAPS", "-" * 58)
    generate_rainfall_map(api_points, flood_risk_list, cyclone_tracks, hybrid_forecast_grid, base_data["lats"], base_data["lons"])
